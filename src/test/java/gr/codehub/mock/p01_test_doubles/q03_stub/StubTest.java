...
public class StubTest {
    ...
    public void demoStubWithMockito() {
        ...
        List<Phone> newPhones = Arrays.asList(
                new Phone("1234", "CodePhone Pro", 600, LocalDate.now()),
                new Phone("1235", "CodePhone Standard", 500, LocalDate.now()));

        when(phoneRepository.findNewPhones(5)).thenReturn(new ArrayList<>());
        when(phoneRepository.findNewPhones(7)).thenReturn(newPhones);
        when(phoneRepository.findNewPhones(anyInt())).thenReturn(newPhones);
